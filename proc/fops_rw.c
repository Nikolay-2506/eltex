...
// смотри -> struct file_operations
static ssize_t node_read( struct file *file, char __user *buf, size_t count, loff_t *ppos )
{
...
static ssize_t node_write( struct file *file, const char __user *buf, size_t count, loff_t *ppos ) {
   char *buf_msg = get_rw_buf();
